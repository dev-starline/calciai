    ...
    public class ClientService
    {
        ...
        private HttpClient InitClient()
        {
            ...
            if (Configuration.TimeOut == 0)
            {
                ...
                Logger.LogTrace($"Time out not found in {Configuration.Name}");
            }
        ...
        public async Task<ProcessResult<T, E>> InvokeAsync<T, E>(HttpMethod methodType, string path, object postData = null) where T : IModel where E : IException
        {
            ...
            {
                Logger.LogError($"HTTP:{result.IsSuccess} - {ex.Message} : payload: {Encoding.UTF8.GetString(result.Response, 0, result.Response.Length)}");
        ...
        public async Task<ApiResponse> InvokeAsync(HttpMethod methodType, string path, object postData = null)
        {
            ...
            try
            {
                ...
                {
                    Logger.LogTrace($"REQUEST: Method: {methodType}, URI: {Configuration.BaseUrl}{path}, Payload: {payload}");
                }
                ...
                if (Logger.IsEnabled(LogLevel.Trace))
                {
                    Logger.LogTrace($"RESPONSE: URL: {request.RequestUri} Code: {httpResponseMessage.StatusCode}, Body: {Encoding.UTF8.GetString(resultBytes, 0, resultBytes.Length)}, Time: {watch.ElapsedMilliseconds}");
                }
                else if (httpResponseMessage.StatusCode != System.Net.HttpStatusCode.OK)
                {
                    Logger.LogError($"RESPONSE: URL: {request.RequestUri} Code: {httpResponseMessage.StatusCode}, Body: {Encoding.UTF8.GetString(resultBytes, 0, resultBytes.Length)}, Time: {watch.ElapsedMilliseconds}");
                }
                else if (watch.ElapsedMilliseconds > 5000)
                {
                    Logger.LogWarning($"RESPONSE: URL: {request.RequestUri} Code: {httpResponseMessage.StatusCode}, Body: {Encoding.UTF8.GetString(resultBytes, 0, resultBytes.Length)}, Time: {watch.ElapsedMilliseconds}");
                }
            ...
            catch (Exception ex)
            {
                ...
                {
                    Logger.LogError($"Error: {ex.Message} in Invoke Method:{path}-ResponseCode:{ex.Message}");
                ...
                Logger.LogError($"exception Type: {ex.GetType().ToString()} in Invoke Method:{path}-ResponseCode:{ex.Message}");
