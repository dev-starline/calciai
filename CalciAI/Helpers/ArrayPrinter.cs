...
{
    public sealed class ArrayPrinter
    {
        ...
        private static int[] GetMaxCellWidths(IEnumerable<dynamic[]> table)
        {
            int maximumCells = 0;
            foreach (var row in from Array row in table
                                where row.Length > maximumCells
                                select row)
            {
        ...
        public static string GetDataInTableFormat(IEnumerable<dynamic[]> table)
        {
            ...
            foreach (Array thisRow in table)
            {
                ...
                {
                    nextRow = table.ElementAt(rowIndex + 1);
